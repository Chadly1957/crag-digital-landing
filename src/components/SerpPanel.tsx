  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-xs text-slate-400 mb-1 block">API Provider</label>
          <select
        ...
        <div>
          <label className="text-xs text-slate-400 mb-1 block">API Key (leave blank for demo)</label>
          <input
            type="password"
            className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500"
      ...
      <button
        ...
        className="w-full py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold text-sm transition-colors"
      >
            ...
            <>
              <div className="flex items-center gap-4 p-4 rounded-lg bg-slate-700/50">
                <div className="text-center">
                  <p className="text-3xl font-black text-white">{data.targetRank ?? "—"}</p>
                  <p className="text-xs text-slate-400 mt-0.5">Current Rank</p>
                </div>
                <div className="flex-1 border-l border-slate-600 pl-4">
                  <p className="text-sm font-semibold text-slate-200">&ldquo;{data.keyword}&rdquo;</p>
                  {data.location && <p className="text-xs text-slate-400">{data.location}</p>}
                  <p className="text-xs text-slate-400 mt-1">{data.totalResults} total results</p>
                  {data.targetRank === null && (
                    <p className="text-xs text-amber-400 mt-1">Not found in top 20 results</p>
                  )}
                </div>
              </div>

              <div className="space-y-1">
                <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Top 10 Results</h4>
                {data.results.slice(0, 10).map(r => (
                  <div
                    key={r.position}
                    className={`flex items-start gap-3 p-2.5 rounded-lg ${r.isTarget ? "bg-blue-500/15 border border-blue-500/30" : "bg-slate-700/30"}`}
                  >
                    <span className={`text-sm font-bold w-6 shrink-0 ${r.isTarget ? "text-blue-400" : "text-slate-400"}`}>#{r.position}</span>
                    <div className="min-w-0">
                      <p className={`text-sm font-medium truncate ${r.isTarget ? "text-blue-300" : "text-slate-200"}`}>{r.title}</p>
                      <p className="text-xs text-slate-400 truncate">{r.url}</p>
                    </div>
                    {r.isTarget && (
                      <span className="shrink-0 text-xs bg-blue-500 text-white px-2 py-0.5 rounded-full font-semibold">YOU</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              {data.relatedKeywords.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Related Keywords</h4>
              ...
              {data.peopleAlsoAsk.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">People Also Ask</h4>
